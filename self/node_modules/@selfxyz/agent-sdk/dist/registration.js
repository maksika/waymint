...
exports.getRegistrationConfigIndex = getRegistrationConfigIndex;
exports.computeRegistrationChallengeHash = computeRegistrationChallengeHash;
exports.signRegistrationChallenge = signRegistrationChallenge;
exports.buildSimpleRegisterUserDataAscii = buildSimpleRegisterUserDataAscii;
exports.buildSimpleDeregisterUserDataAscii = buildSimpleDeregisterUserDataAscii;
exports.buildAdvancedRegisterUserDataAscii = buildAdvancedRegisterUserDataAscii;
exports.buildAdvancedDeregisterUserDataAscii = buildAdvancedDeregisterUserDataAscii;
exports.buildWalletFreeRegisterUserDataAscii = buildWalletFreeRegisterUserDataAscii;
exports.buildSimpleRegisterUserDataBinary = buildSimpleRegisterUserDataBinary;
exports.buildSimpleDeregisterUserDataBinary = buildSimpleDeregisterUserDataBinary;
exports.buildAdvancedRegisterUserDataBinary = buildAdvancedRegisterUserDataBinary;
exports.buildAdvancedDeregisterUserDataBinary = buildAdvancedDeregisterUserDataBinary;
exports.buildWalletFreeRegisterUserDataBinary = buildWalletFreeRegisterUserDataBinary;
 ...
 */
function toConfigIndex(disclosures) {
    const minimumAge = disclosures.minimumAge ?? 0;
    const ofac = disclosures.ofac ?? false;
    if (minimumAge === 18 && ofac)
        return 4;
    if (minimumAge === 21 && ofac)
        return 5;
    if (minimumAge === 18)
        return 1;
    if (minimumAge === 21)
 ...
 */
function assertConfigIndex(idx) {
 ...
 */
function configDigit(configIndex) {
 ...
 */
function numberToOneByte(value) {
...
function bytes32(value) {
    const b = ethers_1.ethers.getBytes(value);
    if (b.length !== 32) {
 ...
 */
function getRegistrationConfigIndex(disclosures = {}) {
 ...
 */
function computeRegistrationChallengeHash(input) {
    ...
    return ethers_1.ethers.keccak256(ethers_1.ethers.solidityPacked(["string", "address", "uint256", "address", "uint256"], ["self-agent-id:register:", human, chainId, registry, nonce]));
}
/**
 ...
 */
async function signRegistrationChallenge(agentPrivateKey, input) {
    const wallet = new ethers_1.ethers.Wallet(agentPrivateKey);
 ...
 */
function buildSimpleRegisterUserDataAscii(disclosures = {}) {
 ...
 * @returns Two-character ASCII userData string.
 */
function buildSimpleDeregisterUserDataAscii(disclosures = {}) {
    const idx = configDigit(toConfigIndex(disclosures));
...
function buildAdvancedDeregisterUserDataAscii(params) {
    const idx = configDigit(toConfigIndex(params.disclosures ?? {}));
    return (ASCII_ACTION.DEREGISTER_ADVANCED + idx + addressHex(params.agentAddress));
}
function buildWalletFreeRegisterUserDataAscii(params) {
    const idx = configDigit(toConfigIndex(params.disclosures ?? {}));
    const guardian = params.guardianAddress
        ? addressHex(params.guardianAddress)
        : "0".repeat(40);
    const sig = signatureParts(params.signature);
    return (ASCII_ACTION.REGISTER_WALLET_FREE +
        idx +
        addressHex(params.agentAddress) +
        guardian +
        sig.r.slice(2).toLowerCase() +
...
function buildSimpleRegisterUserDataBinary(disclosures = {}) {
...
function buildSimpleDeregisterUserDataBinary(disclosures = {}) {
    const idx = toConfigIndex(disclosures);
    assertConfigIndex(idx);
    return ethers_1.ethers.concat([
...
function buildAdvancedRegisterUserDataBinary(params) {
    ...
    return ethers_1.ethers.concat([
        ...
        numberToOneByte(idx),
        ethers_1.ethers.getBytes(normalizeAddress(params.agentAddress)),
        bytes32(sig.r),
        bytes32(sig.s),
        numberToOneByte(sig.v),
    ]);
}
function buildAdvancedDeregisterUserDataBinary(params) {
    const idx = toConfigIndex(params.disclosures ?? {});
    assertConfigIndex(idx);
    return ethers_1.ethers.concat([
...
function buildWalletFreeRegisterUserDataBinary(params) {
    const idx = toConfigIndex(params.disclosures ?? {});
    assertConfigIndex(idx);
    const sig = signatureParts(params.signature);
    const guardian = normalizeAddress(params.guardianAddress ?? ethers_1.ethers.ZeroAddress);
    return ethers_1.ethers.concat([
        numberToOneByte(ACTION.REGISTER_WALLET_FREE),
        numberToOneByte(idx),
        ethers_1.ethers.getBytes(normalizeAddress(params.agentAddress)),
        ethers_1.ethers.getBytes(guardian),
        bytes32(sig.r),
        bytes32(sig.s),
        numberToOneByte(sig.v),
    ]);
